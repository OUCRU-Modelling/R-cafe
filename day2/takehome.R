...
plot_data <- covid_cases %>%
  mutate(country = as.factor(ifelse(country %in% top_countries, 
                                    country, "Others"))) %>% 
  group_by(date, country) %>%
  summarise(total_cases = sum(cases)) %>%
  mutate(
    # country = as.factor(ifelse(country %in% top_countries,
    # country, "Others")),)
    # Em mutate ở đây thì biểu đồ không giống ở task,
    # vì để ở đây thì không gom Others lại chung
    total_cases_per_date = sum(total_cases),
    pct_cases = ifelse(total_cases_per_date == 0, NA, 
...
ggplot(data = plot_data, aes(x = date, y = pct_cases, fill = country)) +
  ...
  scale_fill_discrete(name = "Country",                       
                      breaks = c("chn", "deu", "esp", 
                                 "ita", "usa", "Others"),
                      labels = c("China", "Germany", "Spain", 
                                 "Italy", "USA", "Others")) +
  labs(
