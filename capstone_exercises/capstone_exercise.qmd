...
1.  Load packages necessary for data analysis
...
colnames(joined_data)[colnames(joined_data) == "NA"] <- "SODIUM"
...
8.  Create function to summary characteristics of patients
    ...
    baseline_table <- function(data, intervention) {
      ...
      tbl_summary(
        by = intervention,
        include = c(AGE,SEX,BMI, SOURCE, ICUDAYS, OUTCOME,HYPERTENSION,MYOCARDIALINFART, ANGINA,	PERIVASCULAR,	CHRONICPUL,	CONNECTIVETISSUE,	MILDLIVER, HEMIPLEGIA,DIAWITHCHRONIC,	SEVERELIVER,	AIDS,	CARDIACFAILUREIII,	CARDIACFAILUREIV, CEREBROVASCULAR,	SEVERERESP,	PEPTICULCER,	DIABETES,	SEVEREKIDNEY, MALIGNANCY,	TUMOUR,	DEMENTIA, ELECTIVESURGERY,EMERGENCYSURGERY,TIMETOADM, INCUBATIONPERIOD, INCUPERIODONSET, WOUND, DIFFBREATH,MAXTEMP, GCS,	RESP,	FIO2,	SPO2,	PAO2,	PH,	PLT,	WBC,	HCT, MAXHR,	MINHR,	MAXSBP,	WORSTSBP,	WORSTDBP,	VASO,        BILI,	SODIUM,	K,	CREAT,	URINEOUTPUT,	RENALFAILURE,	IMMUNOCOMPROMISED, ASA, ABLETTSCORE, SOFA_total, TSS_total, APACHE_total),
...
9.  Summary the characteristics of target population
    ...
    baseline_IM_PP <- baseline_table(IM_PP, intervention = "IM_intervention") %>% 
...
12. Primary outcome, requirement for MV

    ```{r}
    tbl_vent <- function(data, intervention) { data %>% 
      ...
      tbl_IT_ITT_vent <- tbl_vent(IT_ITT_vent,  "IT_intervention")
      tbl_IT_ITT_vent
      
      tbl_IM_ITT_vent <- tbl_vent(IM_ITT_vent, "IM_intervention")
      tbl_IM_ITT_vent
      
      tbl_vent_PP <- function(data, intervention) { data %>% 
      mutate(VENSTART = factor(VENSTART, levels= c("Y","N"), labels = c("MV", "No MV"))) %>% 
      tbl_cross(
       row = !!sym(intervention),
       col = VENSTART,
      ...
      tbl_IT_PP_vent <- tbl_vent_PP(IT_PP_vent, "IT_intervention")
      tbl_IT_PP_vent
      tbl_IM_PP_vent <- tbl_vent_PP(IM_PP_vent, "IM_intervention")
      tbl_IM_PP_vent
...
# Summary Adverse Events (AE)
      AE <- read_excel("data/AE.SAE DATA SHEET.xls", sheet = "S_AE_GridAE")
      SAE <- read_excel("data/AE.SAE DATA SHEET.xls", sheet = "S_AE_GridSAE")
      
      # filter events that does not belong to AE
     ...
     AE <- AE %>% 
       ...
       filter(!CTCAENAME %in% c("Nasogastric tube", "Urinary Catheter", "Tracheostomy", "Mechanical ventilation", "ANSD"))
         ...
         # Create the event table for IT_ITT
      AE_IT_ITT <- AE %>% left_join(select(IT_ITT, USUBJID.x, IT_intervention), by = c("USUBJID" = "USUBJID.x"))
      
         # AE summary of Intrathecal treatment arm of IT_ITT
      AE_IT_ITT_Intrathecal <- AE_IT_ITT %>% 
        filter(IT_intervention == "Intrathecal treatment") %>% 
        count(CTCAENAME, name = "Number of events") %>% 
        left_join(
          AE_IT_ITT %>% 
            filter(IT_intervention == "Intrathecal treatment") %>% 
            distinct(CTCAENAME, USUBJID) %>% #loại bỏ các dòng trùng lặp, giữ lại các dòng có giá trị duy nhất vì 1 BN có nhiều tác dụng phụ lặp lại
            count(CTCAENAME, name ="Number of patients"),
          by = "CTCAENAME") %>% 
        arrange(desc(`Number of events`)) %>% # Sắp xếp giảm dần theo số lần xuất hiện
        mutate(CTCAENAME = as.character(CTCAENAME))
      
        # AE summmary of Sham procedure arm of IT_ITT
      AE_IT_ITT_Sham <- AE_IT_ITT %>% 
        filter(IT_intervention == "Sham procedure") %>% 
        count(CTCAENAME, name = "Number of events") %>% 
        left_join(
          AE_IT_ITT %>% 
            filter(IT_intervention == "Sham procedure") %>% 
            distinct(CTCAENAME, USUBJID) %>% 
            count(CTCAENAME, name ="Number of patients"),
          by = "CTCAENAME") %>% 
        arrange(desc(`Number of events`)) %>% 
        mutate(CTCAENAME = as.character(CTCAENAME))
      
       # Count total unique patient:
      
      total_IT_patients <- AE_IT_ITT %>%
      filter(IT_intervention == "Intrathecal treatment") %>%
      distinct(USUBJID) %>%
      nrow()  # Count unique patients with any adverse event

       
      total_Sham_patients <- AE_IT_ITT %>%
      filter(IT_intervention == "Sham procedure") %>%
      distinct(USUBJID) %>%
      nrow()  # Count unique patients with any adverse event
      
      #create the total row for both arm

      Total_IT_row <- tibble(
        CTCAENAME = "Total",
        'Number of events' = sum(AE_IT_ITT_Intrathecal$`Number of events`, na.rm = TRUE),
        'Number of patients' = total_IT_patients)
      
       Total_Sham_row <- tibble( #create the row for the AE table of IT_ITT
        CTCAENAME = "Total",
        'Number of events' = sum(AE_IT_ITT_Sham$`Number of events`, na.rm = TRUE),
        'Number of patients' = total_Sham_patients)
       
       #count total number of patient for both arm
       Total_IT_patient <- IT_ITT %>% 
         group_by(IT_intervention) %>% 
         summarise(total = n_distinct(USUBJID.x), .groups = "drop")
     
       #combine total row to the table
      AE_IT_ITT_Intrathecal <-  bind_rows(AE_IT_ITT_Intrathecal, Total_IT_row) %>% 
        mutate(`Percentage (%)` = sprintf("%d (%.1f%%)", `Number of patients`, `Number of patients`/pull(Total_IT_patient[Total_IT_patient$IT_intervention == "Intrathecal treatment","total"]) *100))

    
      AE_IT_ITT_Sham <-  bind_rows(AE_IT_ITT_Sham, Total_Sham_row) %>%
          mutate(`Percentage (%)` = sprintf("%d (%.1f%%)", `Number of patients`, `Number of patients`/pull(Total_IT_patient[Total_IT_patient$IT_intervention == "Sham procedure","total"])*100))
      
      # combine AE table of both arm into a table
      AE_IT_ITT_table <- full_join(AE_IT_ITT_Intrathecal, AE_IT_ITT_Sham, by = "CTCAENAME", suffix = c("(Intrathecal)", "(Sham)")) %>% 
      mutate(across(where(is.numeric) & (contains("Number of events") | contains("Number of patients")), ~ replace_na(., 0))) %>% 
      mutate(across(contains("Percentage (%)"), ~ replace_na(., "0 (0.0%)"))) %>% 
        rename("Total patients Intrathecal (%)" = "Percentage (%)(Intrathecal)",
               "Total patients Sham (%)" = "Percentage (%)(Sham)",
               "Number of events (Intrathecal)" = "Number of events(Intrathecal)",
               "Number of events (Sham)" = "Number of events(Sham)" ) %>% 
               arrange(if_else(CTCAENAME == "Total", 0, 1))
      
      
      # add p value
      total_patients_intrathecal <-pull(Total_IT_patient[1,2])
      print(total_patients_intrathecal)
      total_patients_sham <-pull(Total_IT_patient[2,2])
      print(total_patients_sham)
      
      AE_IT_ITT_table <- AE_IT_ITT_table %>% 
        rowwise() %>% #tinh theo tung dong
         mutate(
          fisher_p_value = round(fisher.test(
          matrix(
            c(`Number of patients(Intrathecal)`, total_patients_intrathecal - `Number of patients(Intrathecal)`,`Number of patients(Sham)`,total_patients_sham - `Number of patients(Sham)`),
        nrow = 2, byrow = TRUE))$p.value, digits = 3)) %>% 
        ungroup() 
      
      # modify the AE table
   
AE_IT_ITT_table_final <- AE_IT_ITT_table %>% 
  select(CTCAENAME, 
         `Number of events (Intrathecal)`, 
         `Total patients Intrathecal (%)`, 
         `Number of events (Sham)`, 
         `Total patients Sham (%)`, 
         fisher_p_value) %>% 
  gt() %>% 
  ...
  cols_label(
    ...
    `Number of events (Intrathecal)` = md("**n episode**"),
    `Total patients Intrathecal (%)` = md("**n patient**"),
    `Number of events (Sham)` = md("**n episode**"),
    `Total patients Sham (%)` = md("**n patient**"),
    `fisher_p_value` = md("**p value**")
  ) %>%
  tab_spanner(
    label = md(sprintf("**Intrathecal treatment (N=%d)**", total_patients_intrathecal)),
    columns = c(`Number of events (Intrathecal)`, `Total patients Intrathecal (%)`)
  ) %>%
  tab_spanner(
    label = md(sprintf("**Sham procedure (N=%d)**", total_patients_sham)),
    columns = c(`Number of events (Sham)`, `Total patients Sham (%)`)
  ) %>%
  fmt_number(columns = fisher_p_value, decimals = 3) %>%  # Làm tròn giá trị p-value
  tab_options(table.font.size = "medium") %>%
  tab_style(
    style = list(cell_text(weight = "bold")),
    locations = cells_body(rows = CTCAENAME == "Total")  # Bold dòng "Total"
  ) %>% 
  tab_style(
    style = list(cell_text(weight = "bold", color = "red")),  
    locations = cells_body(rows = fisher_p_value < 0.05)  # Tô đỏ và làm đậm dòng có p < 0.05
  )
AE_IT_ITT_table_final
```

15. Convert the long table to wide table, count total dosage of drugs and usage length
    ```{r}
daily_daily<- read_excel(file_path,  sheet = "DAILY_DAILY")
drug_wide <- daily_daily %>%
  pivot_wider(id_cols = USUBJID, names_from = DAILY_DAILY_SEQ, values_from = c("MAXTEMP","MIDAZOLAM", "DIAZEPAMIM",	"DIAZEPAMORAL", "PIPECURONIUM"))

drug_summary <- function(data, drug) {
  data %>% 
    mutate(!!sym(paste0(drug, "_total")) := rowSums(select(., starts_with(drug)), na.rm = TRUE)) %>% 
    mutate(!!sym(paste0(drug, "_length")) := rowSums(!is.na(select(., starts_with(drug))), na.rm = TRUE))
  } #tên cột động

drug_wide_total <- drug_wide %>%   
  drug_summary("PIPECURONIUM") %>% 
  drug_summary("MIDAZOLAM") %>% 
  drug_summary("DIAZEPAMIM") %>% 
  drug_summary("DIAZEPAMORAL") %>% 
  mutate(benzodiazepines_total = DIAZEPAMIM_total + DIAZEPAMORAL_total + (4.17 * MIDAZOLAM_total)) %>% 
  mutate(benzodiazepines_length = pmax(MIDAZOLAM_length, DIAZEPAMIM_length, DIAZEPAMORAL_length, na.rm = TRUE)) %>% 
  mutate(benzodiazepines_total_his = sqrt(benzodiazepines_total)) %>% 
  mutate(PIPECURONIUM_total_his = sqrt(PIPECURONIUM_total))

    ```

16. Create the data frame for each population
```{r}
drug_left_join <- function(population, drug, ID1, ID2) {population %>% 
    left_join(drug, by = setNames(ID2, ID1))}
drug_IT_ITT <- drug_left_join(IT_ITT_vent, drug_wide_total, "USUBJID.x", "USUBJID")
drug_IM_ITT <- drug_left_join(IM_ITT_vent, drug_wide_total, "USUBJID.x", "USUBJID")
drug_IT_PP <- drug_left_join(IT_PP_vent, drug_wide_total, "USUBJID.x", "USUBJID")
drug_IM_PP <- drug_left_join(IM_PP_vent, drug_wide_total, "USUBJID.x", "USUBJID")


pipecuronium_IT_ITT <- drug_IT_ITT %>% 
  select(USUBJID.x, PIPECURONIUM_total_his, PIPECURONIUM_length, VENSTART, IT_intervention) %>% 
  filter(VENSTART == "Y")
```
...
pipecuronium_fit_IT_ITT <- lm(PIPECURONIUM_total_his~IT_intervention, data=pipecuronium_IT_ITT)

total_pipecuronium_IT_ITT <- expand.grid(IT_intervention = unique(pipecuronium_IT_ITT$IT_intervention))
total_pipecuronium_IT_ITT <- cbind(predict(pipecuronium_fit_IT_ITT, newdata=total_pipecuronium_IT_ITT, interval="confidence"), total_pipecuronium_IT_ITT)
total_pipecuronium_IT_ITT$height <- c(0.5,1)

total_pipecuronium_plot_IT_ITT <- ggplot(pipecuronium_IT_ITT)+
  geom_histogram(aes(PIPECURONIUM_total_his,#tổng liều chia theo nhóm điều trị
                     group=IT_intervention,
                     fill=IT_intervention), # tô màu theo nhóm điều trị
                     position="identity", # các cột histogram sẽ chồng lên nhau.
                     alpha=0.4)+ #độ trong suốt
  geom_point(data=total_pipecuronium_IT_ITT,
             aes(fit,height,shape=IT_intervention)) +
  geom_segment(data=total_pipecuronium_IT_ITT,aes(x=lwr,y=height,xend=upr,yend=height,
                                                  linetype=IT_intervention)) +
...
total_pipecuronium_plot_IT_ITT
```
