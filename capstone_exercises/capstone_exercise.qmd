    ...
    # Summary Adverse Events (AE)
     ...
     AE <- AE %>% 
      ...
      mutate(across(where(is.numeric) & (contains("Number of events") | contains("Number of patients")), ~ replace_na(., 0))) %>% 
       ...
       select(-"Number of patients(Intrathecal)") %>% 
        select(-"Number of patients(Sham)") %>% 
      ...
      mutate(`p value` = compute_p_value("Number of patients (Intrathecal)", "Number of patients (Sham)")
